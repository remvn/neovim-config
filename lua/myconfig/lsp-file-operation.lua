...
local has_match = function(fname, filters)
    ...
    return vim.iter(filters or {}):any(
        ...
        function(filter)
            ...
            if
                not match_type
                or (match_type == "file" and is_file)
...
api.events.subscribe(
    ...
    function(payload)
        for _, client in ipairs(vim.lsp.get_clients()) do
            ...
            printDebug(client.name, did_rename)
...
for _, event_created in ipairs({ event.FileCreated, event.FolderCreated }) do
    api.events.subscribe(
        ...
        function(payload)
            ...
            for _, client in ipairs(vim.lsp.get_clients()) do
                ---@type lsp.FileOperationRegistrationOptions?
                local did_create = vim.tbl_get(
                    client,
                    { "server_capabilities", "workspace", "fileOperations", "didCreate" }
                )
                printDebug(client.name, did_create)
...
api.events.subscribe(
    ...
    function(payload)
        for _, client in ipairs(vim.lsp.get_clients()) do
            ...
            printDebug(client.name, will_rename)
...
api.events.subscribe(
    ...
    function(payload)
        for _, client in ipairs(vim.lsp.get_clients()) do
            ...
            printDebug(client.name, will_delete)
...
api.events.subscribe(
    ...
    function(payload)
        for _, client in ipairs(vim.lsp.get_clients()) do
            ...
            printDebug(client.name, will_create)
